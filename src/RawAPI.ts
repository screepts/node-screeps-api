...
export class RawAPI extends EventEmitter {
  ...
  raw: ReturnType<typeof makeInnerAPI>

  constructor(opts: Partial<RawOpts> = {}) {
    super()
    this.opts = {} as any
    this.setServer(opts)
    this.raw = makeInnerAPI(this)
  }
  ...
  async req(method: string, path: string, body: any = {}): Promise<any> {
    ...
    if (res.headers.get("content-type")?.includes("application/json")) {
      ...
      const text = (res.data = await res.text())
      if (text.slice(0, 3) === "gz:") {
        res.data = await this.gz(text)
      }
    }
