...
describe("api.raw.userMessages", function () {
  ...
  describe.skip(".shards.info ()", function () {
    it("should send a request to /api/shards/info and return shards informations", async function () {
      ...
      const res = await api.raw.game.shards.info()
      assert.equal(res.ok, 1, "incorrect server response: ok should be 1")
      assert("shards" in res, "response has no shards field")
      res.shards.forEach((shard, idx) => {
        assert("name" in shard, `shard ${idx} has no name field`)
        assert("rooms" in shard, `shard ${idx} has no rooms field`)
        assert("users" in shard, `shard ${idx} has no users field`)
        assert("tick" in shard, `shard ${idx} has no tick field`)
      })
    })
  })
})
